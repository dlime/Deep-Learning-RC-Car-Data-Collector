...
def load_images(paths, target_size):
    images = np.zeros((len(paths), target_size[0], target_size[1], 3))
...
class RegressionImageDataGenerator(object):
    ...
    def fit(self, X,
        ...
        X = np.copy(X)
        cropped_shape = get_cropped_shape(X.shape[1:], self.cropping)
        cropped = np.zeros((X.shape[0], cropped_shape))
